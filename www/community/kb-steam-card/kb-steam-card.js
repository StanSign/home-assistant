...
function run(node, date$1, localeFunc, opts) {
    ...
    }, Math.max(date.nextInterval(diff), minInterval || 1) * 1000, 0x7fffffff);
...
console.info(`%c  KB-STEAM-CARD \n%c  ${version}   `, 'color: orange; font-weight: bold; background: black', 'color: white; font-weight: bold; background: dimgray');
...
let KbSteamCard = class KbSteamCard extends LitElement {
    ...
    formatLastOnline(lastOnline) {
        return lib_1(new Date(lastOnline));
    }
    renderUserAvatar(entity) {
    ...
    renderCurrentlyPlayingGame(entity) {
